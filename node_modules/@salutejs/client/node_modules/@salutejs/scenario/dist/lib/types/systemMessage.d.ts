...
/**
 * Тип смартапа. Обратите внимание, что ассистент перехватывает навигационные команды 'вверх', 'вниз', 'влево' и
 * 'вправо' только в Canvas App (тип приложения WEB_APP). В этом случае команды обрабатываются на уровне фронтенда
 * приложения. В остальных случаях, команды передаются в бекэнд активного приложения.
 */
 ...
 */
export declare type Surface = 'SBERBOX' | 'COMPANION' | 'STARGATE' | 'SATELLITE' | 'TIME' | 'SBOL' | 'TV' | 'TV_HUAWEI';
/**
...
export declare type Action = TextAction | DeepLinkAction | SendContactPhone | ServerAction | StartSmartSearchAction | CopyTextAction | OpenKeyboard | ShareTextAction;
export declare type Card = DiscoveryCard | ExtendedListCard | GalleryCard | GridCard | ListCard | QRCodeCard | SimpleList | WidgetGallery | WidgetGalleryWithCategories | WidgetSingleCard | WidgetTitleCard | WidgetTwoColumns;
/**
 ...
 */
export declare type Typeface = 'display1' | 'display2' | 'display3' | 'headline1' | 'headline2' | 'headline3' | 'headline4' | 'title1' | 'title2' | 'body1' | 'body2' | 'body3' | 'body_ai' | 'text1' | 'paragraphText1' | 'paragraphText2' | 'footnote1' | 'footnote2' | 'button1' | 'button2' | 'caption' | 'underline';
/**
 ...
 */
export declare type GridContentWidth = 'xsmall' | 'small' | 'premedium' | 'medium' | 'large' | 'xlarge' | 'resizable';
/**
...
export declare type CellView = LeftRightCellView | HorizontalStackCellView | TextCellView | ButtonCellView | ImageCellView | DetailsCellView | WeatherCellView | RightSideCellView | FlexibleImageCellView | HorizontalCellsView;
 ...
 * @minItems 1
 */
export declare type ObjectObject = [DiscoveryCard | ListCard, ...(DiscoveryCard | ListCard)[]];
/**
...
export interface SystemMessage {
    /**
     * Идентификатор сессии, который обновляется каждый раз, когда в поле new_session приходит true. При использовании
     * совместно с messageId помогает гарантировать уникальность сообщения. В том числе если пользователь
     * взаимодействует с несколькими поверхностями.
     */
    sessionId: string;
    /**
...
export interface UUID {
    ...
    userChannel: string;
    /**
     * Постоянный идентификатор пользователя созданный на основе SberID. Может отсутствовать, если пользователь не
     * аутентифицирован. Может использовать для хранения контекста диалога пользователя. Контекст диалога можно
     * обновлять по значению поля new_session.
     */
    sub: string;
    /**
     * Идентификатор, который используется для определения не аутентифицированных пользователей. Идентификатор может
     * изменяться при сбросе настроек или переустановке смартапа.
     */
    userId: string;
    [k: string]: unknown;
}
...
export interface SystemMessagePayload {
    ...
    /**
     * Указывает на характер запуска смартапа. Если поле содержит true, сессии присваивается новый идентификатор (поле
     * sessionId). true — приложение запущено впервые или после закрытия приложения, а так же при запуске приложения по
     * истечению тайм-аута (10 минут) или после прерывания работы приложения, например, по запросу 'текущее время'.
     */
    new_session?: boolean;
    ...
    hints?: Hints;
    /**
     * Сообщает ассистенту о завершении работы смартапа. В приложениях типа Canvas App необходимо самостоятельно
     * закрывать окно приложения после завершения работы смартапа. Для этого требуется передать ассистенту команду
     * close_app с помощью метода assistant.close() или window.AssistantHost.close(), если вы не используете Assistant
     * Client.
     */
    finished?: boolean;
    /**
...
export interface Strategies {
    ...
    last_call?: string;
    /**
     * Передается только в том случае, когда биометрия определила голос Яндекс Алисы. В остальных случаях поле
     * отсутствует.
     */
    is_alice?: boolean;
    [k: string]: unknown;
...
/**
 * Описание элемента экрана, который пользователь назвал при запросе ('включи второй'/'включи второго терминатора'). Для
 * работы этой функциональности нужна отправка во входящем сообщении с фронтенда item_selector со списком элементов.
...
export interface Message {
    /**
     * Исходное сообщение пользователя: распознанный голос или введенный текст. В случае распознанного голоса
     * предоставляется текст запроса без нормализации числительных и другого, соответственно, все числа, номера
     * телефонов и тд представлены словами.
     */
    original_text: string;
    /**
     * Нормализованный текст, который ввел пользователь. Можно использовать для снижения многообразия запросов,
...
export interface AssistantCommand {
    command: SmartAppDataCommand | ActionCommand | CloseAppCommand | PermissionCommand | InvoiceCommand | SmartAppErrorCommand | OperatorCardCommand;
    [k: string]: unknown;
}
/**
...
export interface SendContactPhone {
    ...
    /**
     * Может содержать подстроку [[placeholder]]. Клиент должен подставить в строку шаблона вместо [[placeholder]] номер
     * телефона контакта имеющий id указаный в send_contact_phone
     */
    template?: string;
    [k: string]: unknown;
}
...
export interface TextAttribute {
    ...
    length: number;
    /**
     * Атрибут: цвет символов. Задаётся как HEX c компонентом alpha в виде #AARRGGBB или #RRGGBB. Пустое значение -
...
/**
 * Позиционирование контейнера с контентом
 */
...
/**
 * Вертикальный упорядоченный блок, содержащий опциональные detail и info_box
 * https://www.figma.com/file/MQHBgPkW4dqXmI2549WioI/Dev?node-id=19%3A5686
 */
...
/**
 * Горизнотальный упорядоченный блок, содержащий опциональные info и icon
 * https://www.figma.com/file/MQHBgPkW4dqXmI2549WioI/Dev?node-id=19%3A5686
 */
...
/**
 * Предложения, которые смартап может сделать пользователю в зависимости от контекста диалога. Предложения могут быть
 * представлены в виде кнопок и карточек. Важно! В интерфейсе SberBox предложения носят информационный характер.
...
export interface ASRHints {
    ...
    /**
     * Позволяет выбирать модель распознавания речи в запросе. Если передаётся валидная модель (media или general), то в
     * запросе она может быть изменена. Модель media распознаёт русский и английский языки. Модель general использует
     * только русский язык.
     */
    model?: 'media' | 'general';
 ...
 */
export declare const DefaultChannels: Record<Surface, UserChannel>;
